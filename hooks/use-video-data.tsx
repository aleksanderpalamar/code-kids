...
import { getProgrammingVideos, searchBrazilianProgrammingVideos, searchProgrammingVideos } from "@/lib/youtube-api";
...
export function useVideoData() {
  ...
  const loadInitialVideos = useCallback(async () => {
    try {
      ...
      const initialVideos = await searchBrazilianProgrammingVideos("tutorial programação iniciantes");
      
      // Verificar se o vídeo retornado é um indicador de erro de cota
      if (initialVideos.length === 1 && initialVideos[0].id === "quota-exceeded") {
        setError("Cota da API do YouTube excedida. Tente novamente mais tarde ou contacte o administrador.");
        setVideos([]);
        setFilteredVideos([]);
      } else {
      ...
      setError("Erro ao carregar vídeos. Verifique sua conexão com a internet.");
      setVideos([]);
      setFilteredVideos([]);
    } finally {
      setLoading(false);
  ...
  const handleSearch = async (term: string) => {
    ...
    try {
      ...
      // Verificar se o vídeo retornado é um indicador de erro de cota
      if (searchResults.length === 1 && searchResults[0].id === "quota-exceeded") {
        setError("Cota da API do YouTube excedida. Tente novamente mais tarde ou contacte o administrador.");
        setFilteredVideos([]);
      } else {
